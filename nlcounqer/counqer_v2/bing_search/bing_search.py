...
def call_bing_api(query, count=10, subscription_key='2faf6a70e52a46318ec658b2a14c891c'):
	...
	params = {"q": query, "customconfig": "3701208300", "mkt": "en-US", "safesearch": "Moderate", "responseFilter": "webPages", "count": count}
...
def call_bing_related_searches(query, count=10, subscription_key='cf54d0beb3fc443dac6e1fcf80c9a848'):
	url = "https://api.cognitive.microsoft.com/bing/v7.0/search"
	headers = {'Ocp-Apim-Subscription-Key': subscription_key}
	params = {"q": query, "mkt": "en-US", "responseFilter":"RelatedSearches"}
	response = requests.get(url, headers=headers, params=params)
	...
	if 'relatedSearches' in results:
		for item in results['relatedSearches']['value']:
			relatedSearches.append((item['webSearchUrl'], item['displayText']))
